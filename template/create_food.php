...
include SITE_ROOT.'lib/menu.php';
...
if(empty($_POST['create_food_submit'])) {
  ...
  if($create_food_title_length < 5) {
    var_dump(array('is_error'=>true, 'error'=>array('error_code'=>21, 'error_message'=>'short_firstname', 'error_field'=>'firstname')));
  } else if($create_food_title_length > 17) {
    var_dump(array('is_error'=>true, 'error'=>array('error_code'=>22, 'error_message'=>'long_firstname', 'error_field'=>'firstname')));
  } else if(!preg_match('/^[а-яА-Яёa-zA-Z]*$/u', $create_food_title)) {

  }

  if($create_food_description_length < 5) {
    var_dump(array('is_error'=>true, 'error'=>array('error_code'=>21, 'error_message'=>'short_firstname', 'error_field'=>'firstname')));
  } else if($create_food_description_length > 17) {
    var_dump(array('is_error'=>true, 'error'=>array('error_code'=>22, 'error_message'=>'long_firstname', 'error_field'=>'firstname')));
  } else if(!preg_match('/^[а-яА-Яёa-zA-Z]*$/u', $_POST['create_food_description'])) {
    var_dump(array('is_error'=>true, 'error'=>array('error_code'=>22, 'error_message'=>'long_firstname', 'error_field'=>'firstname')));
...
$uploaddir = 'public/';
$uploadfile = $uploaddir . basename($_FILES['create_food_photo']['name']);

//echo '<pre>';
if (move_uploaded_file($_FILES['create_food_photo']['tmp_name'], $uploadfile)) {
